...
import ChatMessage from "@/components/ChatMessage"; // Reutilizando seu componente oficial
...
// --- SUB-COMPONENTE: CHAT DA SALA (Visual do Chat.tsx) ---
const RoomChatView = ({ roomId, roomName, username, onBack }: { roomId: string, roomName: string, username: string, onBack: () => void }) => {
    ...
    useEffect(() => {
        ...
        socketRef.current.on("receive_message", (msg: any) => {
            // Adaptação para usar o ChatMessage
    ...
    return (
        <div className="flex flex-col h-full bg-zinc-950/50">
            ...
            <div className="flex items-center gap-3 p-3 border-b border-border bg-card/60 backdrop-blur-sm">
                ...
                <div>
                    <h3 className="font-bold text-sm text-foreground">{roomName}</h3>
            ...
            {/* Mensagens - Usando ChatMessage Oficial */}
            ...
            <div className="p-3 border-t border-border bg-card/60 flex gap-2">
                <Input 
                    ...
                    className="h-9 text-sm bg-background/50 border-border focus-visible:ring-primary" 
                />
                <Button size="icon" className="h-9 w-9 gradient-btn border-0" onClick={handleSend}>
...
const SidePanel = ({ username }: { username: string }) => {
    ...
    return (
        <div className="flex flex-col h-full bg-zinc-950/50 border-l border-zinc-800">
            {/* Header da Lista */}
            <div className="p-4 border-b border-zinc-800 bg-card/40">
            ...
            {/* Lista de Salas - Estilo do Rooms.tsx */}
                ...
                <div className="space-y-3">
                    {roomsData.map((room) => (
                        <div 
                            ...
                            className="group flex cursor-pointer items-center gap-3 rounded-xl border border-zinc-800 bg-card/20 p-3 transition-all hover:bg-card/60 hover:border-purple-500/30 hover:shadow-lg"
                        >
                            <div className={`flex h-10 w-10 items-center justify-center rounded-full ${room.bg} ${room.color} ring-1 ring-white/5 group-hover:scale-110 transition-transform`}>
                                <room.icon size={18} />
                            </div>
                            
                            <div className="flex-1 min-w-0">
                                <h3 className="text-sm font-semibold text-zinc-200 group-hover:text-purple-400 transition-colors truncate">
                                    {t(room.nameKey)}
                                </h3>
