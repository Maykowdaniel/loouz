...
// --- FUNÇÃO IA INTELIGENTE (RESPOSTAS) ---
...
// --- FUNÇÃO IA PARA MENSAGEM DE ABERTURA ---
async function getAIOpener(socketId) {
    let botIdentity = botIdentities.get(socketId);
    const userData = textUsers.get(socketId);
    ...
    try {
        const completion = await openai.chat.completions.create({
            ...
            messages: [{
                ...
                YOUR FIXED IDENTITY:
                - Name: ${botIdentity.name}
                - Gender: ${botIdentity.gender}
                - Age: ${botIdentity.age}
                - Location: ${botIdentity.location}
                
                PERSONALITY RULES:
                1. You are bored and lazy. Type in lowercase ONLY.
                2. VERY SHORT answers (1-4 words max).
                3. Be a bit dry/sarcastic. TYPOS ALLOWED.
                Current Topic/Interests: ${interests}.
                
                TASK: Write the FIRST message to start the chat. Be random. Examples of vibe: 'hi', 'm', 'bored', 'sup', 'anyone there?', 'skip?'. DO NOT use quotes.`
            }],
            temperature: 1.2,
            max_tokens: 15,
        });
        ...
        let aiText = completion.choices[0].message.content;
        aiText = aiText.replace(/"/g, '').toLowerCase().trim();
        ...
        console.error("Erro IA Opener:", error.message);
        const botG = botIdentity.gender === 'male' ? 'm' : 'f';
        const fallbacks = ["hi", "hey", "sup", "bored", botG];
...
async function startBotChat(socketId) {
    ...
    // Bot inicia (80% chance de mandar primeiro baseado na sua lógica original)
    if (Math.random() > 0.2) {
        // Delay realista antes de começar a digitar
        const initialHesitation = 1500 + Math.random() * 2000;

        setTimeout(async() => {
            userSocket.emit("partner_typing"); // Mostra que está digitando enquanto a API pensa

            // Chama a IA para gerar a primeira mensagem
            const opener = await getAIOpener(socketId);

            // Calcula o tempo de digitação baseado no tamanho da resposta da IA
            const typingDuration = 600 + (opener.length * 100);
    ...
    socket.on("send_1v1_message", async(data) => {
        ...
        if (pair) {
            if (!pair.isBot) {
                ...
                const readDelay = 1900 + (msgLength * 40) + (Math.random() * 1000);
